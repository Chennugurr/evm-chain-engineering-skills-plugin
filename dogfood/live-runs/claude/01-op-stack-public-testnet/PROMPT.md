...
You are running the v0.3.0-beta strict live evidence smoke test for the local Claude plugin evm-chain-engineering-pro loaded from plugins/evm-chain-engineering-pro.

Use the plugin skills if they are available for this task. If the platform does not expose skill telemetry, state the selected skills in your final answer as observed-by-output/inferred, not as hidden telemetry.

Work only inside dogfood/live-runs/claude/01-op-stack-public-testnet/GENERATED_ARTIFACTS for generated planning artifacts. Do not deploy, broadcast transactions, publish, create real secrets, write the production-network approval marker, edit plugin manifests, or touch unrelated files.
...
python3 scripts/render_artifact_bundle.py --workflow op-stack-public-testnet --stack op-stack --chain-type l2 --environment public-testnet --settlement sepolia --data-availability ethereum-blobs --chain-name smoke-op-stack-public-testnet --output dogfood/live-runs/claude/01-op-stack-public-testnet/GENERATED_ARTIFACTS --overwrite --json-out dogfood/live-runs/claude/01-op-stack-public-testnet/artifact-render-report.json
