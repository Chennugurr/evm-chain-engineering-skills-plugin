# ZK Stack Server Topology

## Purpose

Provide focused guidance for `zk-stack-server-topology.md` within the EVM Chain Engineering Plugin.

## Applies To

Use when the task involves zk, stack, server, topology or adjacent architecture, configuration, operations, or safety decisions.
...
For OP Stack work, separate sequencer, batcher, proposer, challenger, execution client, and consensus client roles. Verify current Optimism docs before using exact op-deployer or client commands. For ZKsync ZK Stack work, distinguish local dummy-executor testing from prover-backed deployments and verify current prover hardware and base token restrictions.
