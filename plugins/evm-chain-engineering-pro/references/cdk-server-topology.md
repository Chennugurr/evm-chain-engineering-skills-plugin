...
Provide focused guidance for `cdk-server-topology.md` within the EVM Chain Engineering Plugin.

## Applies To

Use when the task involves cdk, server, topology or adjacent architecture, configuration, operations, or safety decisions.
...
For OP Stack work, separate sequencer, batcher, proposer, challenger, execution client, and consensus client roles. Verify current Optimism docs before using exact op-deployer or client commands. For Polygon CDK work, identify sovereign, validium, or zkRollup mode, Agglayer connectivity, execution client, privacy requirements, and managed versus self-managed assumptions.
