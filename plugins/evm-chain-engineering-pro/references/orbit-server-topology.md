# Orbit Server Topology

## Purpose

Provide focused guidance for `orbit-server-topology.md` within the EVM Chain Engineering Plugin.

## Applies To

Use when the task involves orbit, server, topology or adjacent architecture, configuration, operations, or safety decisions.
...
For OP Stack work, separate sequencer, batcher, proposer, challenger, execution client, and consensus client roles. Verify current Optimism docs before using exact op-deployer or client commands. For Arbitrum Orbit work, identify the parent chain, Rollup versus AnyTrust mode, validator assumptions, custom gas token constraints, and bridge route before deployment planning.
