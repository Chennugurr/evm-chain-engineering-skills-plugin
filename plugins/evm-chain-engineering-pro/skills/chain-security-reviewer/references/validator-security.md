...
Provide focused guidance for `validator-security.md` within the EVM Chain Engineering Plugin.

## Applies To

Use when the task involves validator, security or adjacent architecture, configuration, operations, or safety decisions.
...
For data availability work, make the DA trust assumption explicit and monitor data publication, retention, retrieval, and fallback paths. For security work, identify assets, actors, admin keys, upgrade powers, emergency controls, and launch blockers before giving a go/no-go result. For validator work, separate consensus keys from administrative keys, define rotation procedures, monitor liveness, and document slashing or jailing risks.
