...
Provide focused guidance for `rpc-security.md` within the EVM Chain Engineering Plugin.

## Applies To

Use when the task involves rpc, security or adjacent architecture, configuration, operations, or safety decisions.
...
For security work, identify assets, actors, admin keys, upgrade powers, emergency controls, and launch blockers before giving a go/no-go result. For RPC work, keep admin/debug endpoints private, rate-limit public methods, monitor latency and error rates, and avoid exposing internal metrics publicly.
