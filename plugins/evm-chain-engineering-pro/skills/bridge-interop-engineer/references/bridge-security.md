...
Provide focused guidance for `bridge-security.md` within the EVM Chain Engineering Plugin.

## Applies To

Use when the task involves bridge, security or adjacent architecture, configuration, operations, or safety decisions.
...
For bridge work, model canonical routes, parent-chain constraints, token mapping, relayer trust, replay protection, withdrawal delays, and TVL limits. For security work, identify assets, actors, admin keys, upgrade powers, emergency controls, and launch blockers before giving a go/no-go result.
