...
COMMAND_KEYS = {"command", "cmd", "text", "content", "patch", "script", "shell"}
CONTAINER_KEYS = {"tool_input", "toolInput", "input", "arguments", "args", "params", "parameters"}


def collect_command_text(value: Any, parts: list[str], *, key: str | None = None) -> None:
    if isinstance(value, str):
        if key in COMMAND_KEYS or key in CONTAINER_KEYS:
            parts.append(value)
        return
    if isinstance(value, dict):
        for child_key, child_value in value.items():
            if child_key in COMMAND_KEYS:
                if isinstance(child_value, str):
                    parts.append(child_value)
                else:
                    collect_command_text(child_value, parts, key=child_key)
            elif child_key in CONTAINER_KEYS:
                collect_command_text(child_value, parts, key=child_key)
    elif isinstance(value, list):
        for child in value:
            collect_command_text(child, parts, key=key)
...
def extract_text_from_event(raw: str) -> str:
    ...
    if isinstance(event, dict):
        collect_command_text(event, parts)
        for key in ("command", "cmd", "text", "content", "patch", "prompt"):
            value = event.get(key)
            if isinstance(value, str):
                parts.append(value)
...
def event_summary(raw: str) -> dict[str, Any]:
    try:
        event = json.loads(raw)
    except json.JSONDecodeError:
        return {"event_parse": "text"}
    if not isinstance(event, dict):
        return {"event_parse": type(event).__name__}
    return {
        "event_parse": "json-object",
        "event_keys": sorted(str(key) for key in event),
        "hook_event_name": event.get("hook_event_name") or event.get("hookEventName"),
        "tool_name": event.get("tool_name") or event.get("toolName") or event.get("tool"),
    }


def write_hook_observation(raw: str, extracted: str, status: str, findings: list[Finding]) -> None:
    log_path = os.environ.get("EVM_CHAIN_HOOK_OBSERVATION_LOG")
    if not log_path:
        return
    payload = {
        "checked_at": None,
        ...
        "extracted_preview": redact(extracted),
        "findings": [
            {
...
def main(argv: list[str] | None = None) -> int:
    ...
    stdin_extracted: str | None = None
    if args.stdin:
        stdin_raw = sys.stdin.read()
        stdin_extracted = extract_text_from_event(stdin_raw)
        findings.extend(scan_text(stdin_extracted, path="stdin"))
    if args.command:
    ...
    if stdin_raw is not None and stdin_extracted is not None:
        write_hook_observation(stdin_raw, stdin_extracted, status, findings)
