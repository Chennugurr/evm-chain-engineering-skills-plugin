...
Use a staged, review-first plan centered on ZK Chain, rollup vs validium, custom base token, sequencer, prover, proof latency, L1 proof submission, bridge, hardware caveats. Exact stack commands and server requirements remain `VERIFY_CURRENT_DOCS` before production use.
...
Cover these required concepts: ZK Chain, rollup vs validium, custom base token, sequencer, prover, proof latency, L1 proof submission, bridge, hardware caveats.
