...
Use a staged, review-first plan centered on sequencer, op-node, execution client, batcher, proposer, RPC topology, bridge, monitoring, firewall, secrets policy, dry-run, launch gates. Exact stack commands and server requirements remain `VERIFY_CURRENT_DOCS` before production use.
...
Cover these required concepts: sequencer, op-node, execution client, batcher, proposer, RPC topology, bridge, monitoring, firewall, secrets policy, dry-run, launch gates.
